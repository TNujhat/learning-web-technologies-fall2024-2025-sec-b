...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Optional: Handle file upload (for photos)
    $photos = $_FILES['photos']['name'];
    $photos_tmp = $_FILES['photos']['tmp_name'];
    $upload_dir = "../uploads/accident_photos/"; // Folder where the photos will be saved
    ...
    // Array to store paths of uploaded photos
    $uploaded_photos = [];
    ...
    if (empty($accident_date_time) || empty($location) || empty($vehicle_number) || empty($description) || empty($contact)) {
        ...
        if ($conn->connect_error) {
            ...
            // Convert the array of file paths to a string for storage
